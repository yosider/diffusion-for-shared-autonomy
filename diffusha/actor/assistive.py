...
class DiffusionAssistedActor(Actor):
    def __init__(
        ...
        behavioral_actor: Actor = None,
        fwd_diff_ratio: float = 0.45,
    ) -> None:
        ...
        print(
            f"forward diffusion steps for action: "
            f"{self._k} / {self.diffusion.num_diffusion_steps}"
        )
    ...
    def _diffusion_cond_sample(self, obs, user_act, run_in_batch=False):
        ...
        if torch.is_tensor(obs):
            ...
            # Luzhe: TEMP!
            # This if else condition is specific for play.py
            # I am not sure whether this would cause a problem for eval
            state = torch.as_tensor(np.concatenate((obs, user_act), axis=0))

        # NOTE: Currently only support hard conditioning
        # (replacing a part of the input / output)

        # Forward diffuse user_act for k steps
        if not run_in_batch:
            x_k, e = self.diffusion.diffuse(
                state.unsqueeze(0), torch.as_tensor([self._k])
            )
        else:
            x_k, e = self.diffusion.diffuse(state, torch.as_tensor([self._k]))

        # Reverse diffuse Tensor([*crisp_obs, *noisy_user_act])
        # for (diffusion.num_diffusion_steps - k) steps
    ...
    def act(
        self, obs: np.ndarray, report_diff: bool = False, return_original: bool = False
    ):
    ...
    def batch_act(
        self, obss: np.ndarray, report_diff: bool = False, return_original: bool = False
    ):
        user_actions = []
        obs_copilots = []
        obs_pilots = []
        ...
        if self.fwd_diff_ratio != 0:
            ...
            actions = self._diffusion_cond_sample(
                obs_copilots, user_actions, run_in_batch=True
            )
        else:
        ...
        if report_diff:
            # TEMP: LUZHE suppose they are in shape (n * (action_dim+obs_dim)) ,
            # maybe flipped, take care about axis
    ...
    def act_without_env(
        self, obs: np.ndarray, act: np.ndarray, report_diff: bool = False
    ):
        if isinstance(obs, dict):
            obs_pilot = obs["pilot"]
            obs_copilot = obs["copilot"]
