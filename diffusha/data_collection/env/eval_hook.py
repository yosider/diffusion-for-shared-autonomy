...
def get_frame(
    ...
    if "Push" not in env_name:
        ...
        if obs is not None:
            ...
            frame = cv2.putText(
                frame,
                f"obs[-2], obs[-1]: {obs[-2]:.3f}, {obs[-1]:.3f}",
                org=(0, 220),
                fontFace=3,
                fontScale=fontScale,
                color=(0, 255, 0),
        ...
        if info is not None:
            frame = cv2.putText(
                frame,
                f'game-over: {info.get("game_over_reason", "")}',
                org=(0, 260),
                fontFace=3,
                fontScale=fontScale,
                color=(0, 255, 0),
            ...
            frame = cv2.putText(
                frame,
                f'timelimit: {info.get("TimeLimit.truncated", "")}',
                org=(0, 300),
                fontFace=3,
                fontScale=fontScale,
                color=(0, 255, 0),
...
def get_eval_frame(
    ...
    if is_ll:
        if info is not None:
            success = "goal" in info.keys() and (
                info["goal"] == "landed" or info["goal"] == "target-reached"
            )
            if success:
...
class WandBLogger(EvaluationHook):
    ...
    def __call__(
        self, env, agent, evaluator, step, eval_stats, agent_stats, env_stats
    ) -> None:
        ...
        with agent.eval_mode():
            for ep in range(num_eval_episodes):
                ...
                while not done:
                    ...
                    frames.append(
                        get_frame(
                            env, ep, step, obs, rew, rew_sum, action=action, info=info
                        )
                    )
        ...
        wandb.log(
            {
                ...
                "eval/video": wandb.Video(
                    np.asarray(frames).transpose(0, 3, 1, 2), fps=30, format="mp4"
                ),
                "step": step,
            }
        )
        ...
        # [
        #     ("average_q1", 1.5966111),
        #     ("average_q2", 1.6748627),
        #     ("average_q_func1_loss", 0.1904826650226658),
        #     ("average_q_func2_loss", 0.30078676901757717),
        #     ("n_updates", 44),
        #     ("average_entropy", 1.3570675),
        #     ("temperature", 0.986836314201355),
        # ]
