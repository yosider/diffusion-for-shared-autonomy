...
def set_egl_id():
    ...
    if (
        os.environ.get("CUDA_VISIBLE_DEVICES", False)
        and "EGL_DEVICE_ID" not in os.environ
    ):
        print(
            "pre EGL_DEVICE_ID", os.environ.get("EGL_DEVICE_ID", "variable not found")
        )
        ...
        print(
            ...
            os.environ.get("CUDA_VISIBLE_DEVICES", "variable not found"),
        )
...
def report_cuda_error(job_name):
    """
    Checks if torch.cuda.is_available() otherwise,
    report the jobname to ~/cuda-error.txt
    """
    import os
