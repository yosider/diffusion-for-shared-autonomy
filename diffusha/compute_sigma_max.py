...
beta_mins = [1e-4]
beta_maxs = [0.02, 0.1, 0.2, 0.4, 0.51]

for beta_min in beta_mins:
    for beta_max in beta_maxs:
        for T in timesteps:
            betas = make_beta_schedule(
                schedule="sigmoid", n_timesteps=T, start=1e-4, end=beta_max
            )
            alphas = 1 - betas
            ...
            print(
                f"timesteps: {T}\tbeta_min: {beta_min}\tbeta_max: {beta_max}"
                f"\tsigma_min: {sigmas[0]}\tsigma_max: {sigmas[-1]}"
            )
